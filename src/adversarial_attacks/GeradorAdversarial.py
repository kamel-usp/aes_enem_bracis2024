def gerar_ataque(ataque):
    ...
    frases_fluentes = [
        "Inegavelmente, progredir lentamente, vagarosamente, cuidadosamente, silenciosamente enquanto respira profundamente e pensa intensamente sobre o problema em questão. ",
...
for a in ataque:
    ...
    for t in frases:
        print(t)
        response = calcula_features2(t)
        lista.append(response)
    df = pd.DataFrame.from_dict(lista) 
    df.to_csv("Ataque_adv_"+a+".csv", encoding='utf-8', index=False)
"""
